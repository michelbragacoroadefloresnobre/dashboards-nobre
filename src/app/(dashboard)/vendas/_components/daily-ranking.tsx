...
import { AVATAR_COLORS, dailyData } from "@/data/dashboard";
...
// Row height: py-[9px]*2 + content ~30px + gap ~2px ≈ 50px
const ROW_HEIGHT = 50;
...
export function DailyRanking() {
  ...
  const totalPages = Math.ceil(dailyData.length / effectivePerPage);
  const carousel = useCarousel(totalPages);

  const pages = Array.from({ length: totalPages }, (_, p) => {
    const slice = dailyData.slice(
      p * effectivePerPage,
      ...
      <div key={p} className="flex flex-col gap-0.5">
        {slice.map((v) => {
          ...
          return (
            <div
              key={v.pos}
              className="grid grid-cols-[24px_32px_1fr_auto_auto] items-center gap-2.5 py-[9px] px-3 rounded-[10px]"
            >
              <div
                className={`text-xs font-bold text-center ${posColor(v.pos)}`}
              >
                {v.pos}
              </div>
              <div
                className="w-[30px] h-[30px] rounded-lg flex items-center justify-center text-xs font-semibold text-white"
                style={{
              ...
              <div className="text-xs text-text-secondary text-right min-w-[48px]">
                {v.orders}
              </div>
            </div>
          );
    ...
    <div className="bg-bg-card rounded-2xl border border-border shadow-[0_1px_3px_rgba(0,0,0,0.04),0_1px_2px_rgba(0,0,0,0.03)] flex flex-col flex-1 overflow-hidden">
      <div className="flex items-center justify-between px-[22px] pt-[18px] shrink-0">
        <div className="text-xs font-semibold uppercase tracking-[1.2px] text-text-secondary flex items-center gap-2">
          <span className="text-[15px]">📅</span> Desempenho Diário
        </div>
        <div className="text-[11px] px-2.5 py-[3px] rounded-full font-medium bg-accent-gold-light text-accent-gold">
          Hoje
        </div>
      </div>
      <div className="px-[22px] pt-4 pb-[22px] flex-1 flex flex-col min-h-0">
        <div className="grid grid-cols-[24px_32px_1fr_auto_auto] gap-2.5 px-3 pb-2 border-b border-border-light mb-1 shrink-0">
